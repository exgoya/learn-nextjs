...
async def create_user(user: _schemas.UserCreate, db: _orm.Session = _fastapi.Depends(_services.get_db)):
    ...
    if db_user:
        raise _fastapi.HTTPException(status_code=400,detail="Email already in used")
...
async def update_lead(lead_id: int, 
                      ...
                      user: _schemas.User=_fastapi.Depends(_services.get_current_user), db: _orm.Session=_fastapi.Depends(_services.get_db)):
